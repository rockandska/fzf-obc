...
@test "__fzf_obc::config::get::create should create __fzf_obc::config::get with the right config" {
	create_cfg_files4tests
	run __fzf_obc::config::get::create "${BATS_TEST_TMPDIR}/.config/fzf-obc" "${BATS_TEST_TMPDIR}/.config/fzf-obc/plugins" "${BATS_TEST_TMPDIR}/.config/fzf-obc/plugins/subplugins"
	[ "$status" -eq 0 ]
	[ "$output" == "" ]
	local to_check=$(declare -f __fzf_obc::config::get | sed -n '/case "${cfg_level}" in/,${p;/esac;/q}')
