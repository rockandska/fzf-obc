...
@test "__fzf_obc::completion::findshortest should set correct prefix when ignore_case is off" {
	...
	run __fzf_obc::completion::findshortest "abcd" COMPREPLY var
	[ "$status" -eq 0 ]
	[ "$output" == "" ]
	echo "var = $var"
	[ "${var}" == "abcd" ]
...
@test "__fzf_obc::completion::findshortest should set correct prefix when ignore_case is on" {
	if [[ "${BASH_VERSINFO[0]}" -lt 4 ]];then
		# Can't use bind command without interactive shell with bash < 4
		__fzf_obc::readline::completion_ignore_case() {
			return 0
	...
	local var
	local COMPREPLY=(abcd aBc aB)
	run __fzf_obc::completion::findshortest "aBc" COMPREPLY var
	[ "$status" -eq 0 ]
	[ "$output" == "" ]
	echo "var = $var"
	[ "${var}" == "aBc" ]
